...
namespace OnlineFood.Web.Areas.Admin.Controllers
{
    public class RestaurantCategoryController : BaseController
    {
        ...
        public RestaurantCategoryController(IUnitOfWork unitOfWork, IRestaurantCategoryService restaurantCategoryService)
        {
        ...
        [HttpPost]
        public JsonResult GetData(jQueryDataTableParamModel param, SearchRestaurantCategoryViewModel filter)
        {
            ...
            var query = _restaurantCategoryService.GetAll();
        ...
        [HttpPost]
        public async Task<ActionResult> Create(CreateRestaurantCategoryViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                var model = new RestaurantCategory()
                {
                    Title = viewModel.Title,
                    ActivityStatus = viewModel.ActivityStatus,
                };
                _restaurantCategoryService.Add(model);
        ...
        {
            var model = await _restaurantCategoryService.GetAsync(x => x.Id == id, asNoTracking: true);
            var viewModel = new EditRestaurantCategoryViewModel()
            {
                Id = model.Id,
                Title = model.Title,
        ...
        [HttpPost]
        public async Task<ActionResult> Edit(EditRestaurantCategoryViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                var model = await _restaurantCategoryService.GetAsync(x => x.Id == viewModel.Id);
        ...
        public async Task<ActionResult> Delete(int id)
        {
            ...
            try
            {
                int result = await _restaurantCategoryService.DeleteAsync(w => w.Id == id);
                if (result >= 1)
                {
