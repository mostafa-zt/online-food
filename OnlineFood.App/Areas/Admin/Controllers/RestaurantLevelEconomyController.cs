...
{
    public class RestaurantLevelEconomyController : BaseController
    {
        ...
        public RestaurantLevelEconomyController(IUnitOfWork unitOfWork, IRestaurantLevelEconomyService restaurantLevelEconomyService)
        {
        ...
        [HttpPost]
        public JsonResult GetData(jQueryDataTableParamModel param, SearchRestaurantLevelEconomyViewModel filter)
        {
            ...
            var query = _restaurantLevelEconomyService.GetAll();
        ...
        [HttpPost]
        public async Task<ActionResult> Create(CreateRestaurantLevelEconomyViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                var model = new RestaurantLevelEconomy()
                {
                    Title = viewModel.Title,
                    ActivityStatus = viewModel.ActivityStatus,
                };
                _restaurantLevelEconomyService.Add(model);
        ...
        {
            var model = await _restaurantLevelEconomyService.GetAsync(x => x.Id == id, asNoTracking: true);
            var viewModel = new EditRestaurantLevelEconomyViewModel()
            {
                Id = model.Id,
                Title = model.Title,
        ...
        [HttpPost]
        public async Task<ActionResult> Edit(EditRestaurantLevelEconomyViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                var model = await _restaurantLevelEconomyService.GetAsync(x => x.Id == viewModel.Id);
        ...
        public async Task<ActionResult> Delete(int id)
        {
            ...
            try
            {
                int result = await _restaurantLevelEconomyService.DeleteAsync(w => w.Id == id);
                if (result >= 1)
                {
