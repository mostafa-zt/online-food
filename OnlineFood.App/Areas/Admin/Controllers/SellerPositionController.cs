...
{
    public class SellerPositionController : BaseController
    {
        #region Private Properties and Constructor

        private readonly IUnitOfWork _unitOfWork;
        private readonly ISellerPositionService _positionTypeService;

        public SellerPositionController(IUnitOfWork unitOfWork, ISellerPositionService positionTypeService)
        {
        ...
        [HttpPost]
        public JsonResult GetData(jQueryDataTableParamModel param, SearchSellerPositionViewModel filter)
        {
            ...
            var query = _positionTypeService.GetAll();
        ...
        [HttpPost]
        public async Task<IActionResult> Create(CreateSellerPositionViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                var model = new SellerPosition()
                {
                    Title = viewModel.Title,
                    ActivityStatus = viewModel.ActivityStatus,
                };
                _positionTypeService.Add(model);
        ...
        {
            var model = await _positionTypeService.GetAsync(x => x.Id == id, asNoTracking: true);
            var viewModel = new EditSellerPositionViewModel()
            {
                Id = model.Id,
                Title = model.Title,
        ...
        [HttpPost]
        public async Task<IActionResult> Edit(EditSellerPositionViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                var model = await _positionTypeService.GetAsync(x => x.Id == viewModel.Id);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            try
            {
                int result = await _positionTypeService.DeleteAsync(w => w.Id == id);
                if (result >= 1)
                {
