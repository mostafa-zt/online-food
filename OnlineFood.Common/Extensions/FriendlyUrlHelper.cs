...
namespace OnlineFood.Common.Extensions
{
    ...
    public static class FriendlyUrlHelper
    {
        ...
        /// }
        /// </code>
        public static string GetFriendlyTitle(this string title, bool remapToAscii = false, int maxlength = 80)
        {
            if (title == null)
            {
                return string.Empty;
        ...
        {
            return url.Trim().Replace(" ", "-").Replace(".", "-").Replace("/", "-").Replace("?", "-").Replace("*", "-");
        }
        public static string ToRecoverUrlString(this string url)
        {
            return url.Trim().Replace("-", " ").Replace("-", ".").Replace("-", "/").Replace("-", "?").Replace("-", "*");
        }
